...
from odrive.utils import Event, Logger
...
def for_all_parallel(objects, get_name, callback):
    ...
    if len(tracebacks) == 1:
        msg = f"task {tracebacks[0][0]} failed."
        raise Exception(msg) from tracebacks[0][1]
    elif len(tracebacks) > 1:
        msg = "task {} and {} failed.".format(
            tracebacks[0][0], "one other" if len(tracebacks) == 2 else str(len(tracebacks) - 1) + " others"
        )
        raise Exception(msg) from tracebacks[0][1]
...
parser.add_argument(
    "--skip-boring-tests", action="store_true", help="Skip the boring tests and go right to the high power tests"
)
parser.add_argument("--ignore", metavar="DEVICE", action="store", nargs="+", help="Ignore one or more ODrives or axes")
parser.add_argument("--test-rig-yaml", type=argparse.FileType("r"), help="test rig YAML file")
...
if test_rig_yaml["type"] == "parallel":
    ...
    all_tests.append(TestSelfLoadedPosVelDistribution(rpm_range=3000, load_current_range=60, driver_current_lim=70))
...
for odrv_idx, odrv_yaml in enumerate(test_rig_yaml["odrives"]):
    name = odrv_yaml["name"] if "name" in odrv_yaml else f"odrive{odrv_idx}"
        ...
        if isinstance(test, ODriveTest):
            ...
            def dual_axis_test_thread(coupling, test=test):
                ...
                try:
                    if not app_shutdown_token.is_set():
                        # Run test on this axis
                        logger.notify(f"* running {type(test).__name__} on {coupling_name}...")
                        try:
