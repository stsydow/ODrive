...
for axisID in range(8):
    ...
    # 0x001 - Heartbeat
    axisError = can.Signal(
        "Axis_Error", 0, 32, receivers=["Master"], choices={error.value: error.name for error in AxisError}
    )
    axisState = can.Signal(
        "Axis_State", 32, 8, receivers=["Master"], choices={state.value: state.name for state in AxisState}
    )
    ...
    heartbeatMsg = can.Message(
        ...
        "Heartbeat",
        8,
        [axisError, axisState, motorErrorFlag, encoderErrorFlag, controllerErrorFlag, trajectoryDoneFlag],
        send_type="cyclic",
        cycle_time=100,
    ...
    # 0x003 - Motor Error
    motorError = can.Signal(
        "Motor_Error", 0, 32, receivers=["Master"], choices={error.value: error.name for error in MotorError}
    )
    motorErrorMsg = can.Message(0x003, "Get_Motor_Error", 8, [motorError], senders=[newNode.name])

    # 0x004 - Encoder Error
    encoderError = can.Signal(
        "Encoder_Error", 0, 32, receivers=["Master"], choices={error.value: error.name for error in EncoderError}
    )
    encoderErrorMsg = can.Message(0x004, "Get_Encoder_Error", 8, [encoderError], senders=[newNode.name])

    # 0x005 - Sensorless Error
    sensorlessError = can.Signal(
        "Sensorless_Error",
        0,
        32,
        receivers=["Master"],
        choices={error.value: error.name for error in SensorlessEstimatorError},
    )
    sensorlessErrorMsg = can.Message(0x005, "Get_Sensorless_Error", 8, [sensorlessError], senders=[newNode.name])
    ...
    axisRequestedState = can.Signal(
        "Axis_Requested_State",
        0,
        32,
        receivers=[newNode.name],
        choices={state.value: state.name for state in AxisState},
    )
    setAxisState = can.Message(0x007, "Set_Axis_State", 8, [axisRequestedState], senders=["Master"])
    ...
    encoderEstimates = can.Message(
        ...
        8,
        [encoderPosEstimate, encoderVelEstimate],
        senders=[newNode.name],
        send_type="cyclic",
        cycle_time=10,
    )
    ...
    controlMode = can.Signal(
        "Control_Mode", 0, 32, receivers=[newNode.name], choices={state.value: state.name for state in ControlMode}
    )
    inputMode = can.Signal(
        "Input_Mode", 32, 32, receivers=[newNode.name], choices={state.value: state.name for state in InputMode}
    )
    setControllerModeMsg = can.Message(0x00B, "Set_Controller_Mode", 8, [controlMode, inputMode], senders=["Master"])
    ...
    setInputPosMsg = can.Message(0x00C, "Set_Input_Pos", 8, [inputPos, velFF, torqueFF], senders=["Master"])
    ...
    setInputVelMsg = can.Message(0x00D, "Set_Input_Vel", 8, [inputVel, inputTorqueFF], senders=["Master"])

    # 0x00E - Set Input Torque
    inputTorque = can.Signal("Input_Torque", 0, 32, is_float=True, receivers=[newNode.name], unit="Nm")
    setInputTqMsg = can.Message(0x00E, "Set_Input_Torque", 8, [inputTorque], senders=["Master"])
    ...
    setVelLimMsg = can.Message(0x00F, "Set_Limits", 8, [velLimit, currentLimit], senders=["Master"])
    ...
    setTrajVelMsg = can.Message(0x011, "Set_Traj_Vel_Limit", 8, [trajVelLim], senders=["Master"])
    ...
    setTrajAccelMsg = can.Message(0x012, "Set_Traj_Accel_Limits", 8, [trajAccelLim, trajDecelLim], senders=["Master"])

    # 0x013 - Set Traj Inertia
    trajInertia = can.Signal("Traj_Inertia", 0, 32, is_float=True, receivers=[newNode.name], unit="Nm / (rev/s^2)")
    trajInertiaMsg = can.Message(0x013, "Set_Traj_Inertia", 8, [trajInertia], senders=["Master"])
    ...
    sensorlessPosEstimate = can.Signal(
        "Sensorless_Pos_Estimate", 0, 32, is_float=True, receivers=["Master"], unit="rev"
    )
    sensorlessVelEstimate = can.Signal(
        "Sensorless_Vel_Estimate", 32, 32, is_float=True, receivers=["Master"], unit="rev/s"
    )
    ...
    velIntGain = can.Signal(
        "Vel_Integrator_Gain", 32, 32, is_float=True, receivers=[newNode.name], unit="(Nm / (rev/s)) / s"
    )
    ...
    controllerError = can.Signal(
        "Controller_Error", 0, 32, receivers=["Master"], choices={error.value: error.name for error in ControllerError}
    )
    controllerErrorMsg = can.Message(0x01D, "Get_Controller_Error", 8, [controllerError], senders=[newNode.name])
