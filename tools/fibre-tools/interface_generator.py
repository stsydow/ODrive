...
def to_pascal_case(s):
    return "".join([(w.title() if w not in dictionary else w) for w in get_words(s)])


def to_camel_case(s):
    return "".join([(c.lower() if i == 0 else c) for i, c in enumerate("".join([w.title() for w in get_words(s)]))])


def to_macro_case(s):
    return "_".join(get_words(s)).upper()
...
value_types = OrderedDict(
    {
        ...
        "endpoint_ref": {
            "builtin": True,
            "fullname": "endpoint_ref",
            "name": "endpoint_ref",
            "c_name": "endpoint_ref_t",
            "py_type": "[not implemented]",
        },
    }
)
...
def regularize_func(path, name, elem, prepend_args):
    ...
    elem["fullname"] = path = join_name(path, name)
    elem["in"] = OrderedDict(
        (n, regularize_arg(path, n, arg)) for n, arg in (*prepend_args.items(), *get_dict(elem, "in").items())
    )
    elem["out"] = OrderedDict((n, regularize_arg(path, n, arg)) for n, arg in get_dict(elem, "out").items())
    return elem
...
def regularize_attribute(parent, name, elem, c_is_class):
    ...
    if isinstance(elem["type"], str) and elem["type"].startswith("readonly "):
        ...
        if elem["typeargs"]["fibre.Property.mode"] == "readonly" and "c_setter" in elem:
            elem.pop("c_setter")
        ...
        elem["typeargs"]["fibre.Property.type"] = regularize_valuetype(
            parent.fullname, to_pascal_case(name), elem["type"]
        )
        elem["type"] = InterfaceRefElement(parent.fullname, None, "fibre.Property", elem["typeargs"])
        if elem["typeargs"]["fibre.Property.mode"] == "readonly" and "c_setter" in elem:
            elem.pop("c_setter")
...
class InterfaceRefElement:
    ...
    def resolve(self):
        ...
        raise Exception(
            f"could not resolve type {self._name} in {self._scope}. Known interfaces are: {list(interfaces.keys())}. Known value types are: {list(value_types.keys())}"
        )
    ...
    def __init__(self, name, fullname, mode, value_type):
        ...
        if mode != "readonly":
            self.functions["exchange"] = {
                ...
                "in": OrderedDict(
                    [
                        ("obj", {"name": "obj", "type": {"c_name": self.c_name}}),
                        ("value", {"name": "value", "type": value_type, "optional": True}),
                    ]
                ),
...
def regularize_valuetype(path, name, elem):
    ...
    if "flags" in elem:  # treat as flags
        ...
        if "nullflag" in elem:
            elem["flags"] = OrderedDict(
                [
                    (elem["nullflag"], OrderedDict({"name": elem["nullflag"], "value": 0, "bit": None})),
                    *elem["flags"].items(),
                ]
            )
...
def resolve_valuetype(scope, name):
    ...
    raise Exception(
        f"could not resolve type {name} in {join_name(*scope)}. Known value types are: {list(value_types.keys())}"
    )
...
def generate_endpoint_table(intf, bindto, idx):
    ...
    for k, prop in intf.get_all_attributes().items():
        ...
        if len(property_value_type):
            ...
            inner_endpoints, inner_endpoint_definitions, inner_cnt = generate_endpoint_table(
                prop["type"], attr_bindto, idx + cnt
            )
            endpoints += inner_endpoints
            endpoint_definitions.append({"name": k, "type": "object", "members": inner_endpoint_definitions})
            cnt += inner_cnt
    ...
    for k, func in intf.get_all_functions().items():
        endpoints.append(
            {
                "id": idx + cnt,
                "function": func,
                "in_bindings": OrderedDict(
                    [
                        ("obj", bindto),
                        *[
                            (k_arg, "(" + bindto + ")->" + func["name"] + "_in_" + k_arg + "_")
                            for k_arg in list(func["in"].keys())[1:]
                        ],
                    ]
                ),
                "out_bindings": OrderedDict(
                    (k_arg, "&(" + bindto + ")->" + func["name"] + "_out_" + k_arg + "_")
                    for k_arg in func["out"].keys()
                ),
            }
        )
        in_def = []
        ...
        for i, (k_arg, arg) in enumerate(list(func["in"].items())[1:]):
            endpoint, endpoint_definition = generate_endpoint_for_property(
                {
                    "name": arg["name"],
                    "type": make_property_type(
                        {"fibre.Property.type": arg["type"], "fibre.Property.mode": "readwrite"}
                    ),
                },
        ...
        for i, (k_arg, arg) in enumerate(func["out"].items()):
            endpoint, endpoint_definition = generate_endpoint_for_property(
                ...
                intf.c_name + "::get_" + func["name"] + "_out_" + k_arg + "_" + "(" + bindto + ")",
                idx + cnt + len(func["in"]) + i,
            )
            endpoints.append(endpoint)
            out_def.append(endpoint_definition)

        endpoint_definitions.append(
            {"name": k, "id": idx + cnt, "type": "function", "inputs": in_def, "outputs": out_def}
        )
...
parser.add_argument("-v", "--verbose", action="store_true", help="print debug information (on stderr)")
parser.add_argument(
    "-d",
    "--definitions",
...
parser.add_argument("-t", "--template", type=argparse.FileType("r", encoding="utf-8"), help="the code template")
group = parser.add_mutually_exclusive_group(required=True)
group.add_argument("-o", "--output", type=argparse.FileType("w", encoding="utf-8"), help="path of the generated output")
group.add_argument(
    "--outputs",
    type=str,
...
if len(clashing_names):
    print(
        f"**Error**: Found both an interface and a value type with the name {clashing_names[0]}. This is not allowed, interfaces and value types (such as enums) share the same namespace.",
        file=sys.stderr,
    )
    sys.exit(1)
...
if args.generate_endpoints:
    endpoints, embedded_endpoint_definitions, _ = generate_endpoint_table(
    ...
    def token_transform(token):
        ...
        for name in token_list:
            ...
            if (not name.endswith("()")) and name in scope.get_all_attributes():
                ...
                print(f'Warning: cannot resolve "{token}" in {intf.fullname}')
                return "`" + token + "`"
...
env.filters["to_c_string"] = lambda x: "\n".join(
    ('"' + line.replace('"', '\\"') + '"')
    for line in json.dumps(x, separators=(",", ":")).replace('{"name"', '\n{"name"').split("\n")
)
