...
def dump_errors(odrv, clear=False, printfunc=print):
    ...
    def dump_errors_for_module(indent, name, obj, path, errorcodes):
        ...
        if obj != 0:
            ...
            for bit in range(64):
                if obj & (1 << bit) != 0:
                    printfunc(indent + "  " + errorcodes.get((1 << bit), f"UNKNOWN ERROR: 0x{1 << bit:08X}"))
        else:
    ...
    for name, axis in axes:
        ...
        module_decode_map = [
            ...
            (
                ...
                axis,
                "motor.error",
                {v: k for k, v in odrive.enums.__dict__.items() if k.startswith("MOTOR_ERROR_")},
            ),
            (
                "sensorless_estimator",
                axis,
                "sensorless_estimator.error",
                {v: k for k, v in odrive.enums.__dict__.items() if k.startswith("SENSORLESS_ESTIMATOR_ERROR")},
            ),
            (
                "encoder",
                axis,
                "encoder.error",
                {v: k for k, v in odrive.enums.__dict__.items() if k.startswith("ENCODER_ERROR_")},
            ),
            (
                "controller",
                axis,
                "controller.error",
                {v: k for k, v in odrive.enums.__dict__.items() if k.startswith("CONTROLLER_ERROR_")},
            ),
        ]
    ...
    def __init__(self, get_var_callback, data_rate=500.0, duration=2.0):
        ...
        def loop():
            ...
            if achieved_data_rate < (data_rate * 0.9):
                ...
                print(
                    "If this rate is significantly lower than what you specified, consider lowering it "
                    "below the achieved value for more consistent sampling."
                )
...
def step_and_plot(axis, step_size=100.0, settle_time=0.5, data_rate=500.0, ctrl_mode=CONTROL_MODE_POSITION_CONTROL):
    ...
    capture = BulkCapture(get_var_callback, data_rate=data_rate, duration=initial_settle_time + settle_time)
...
def dump_dma(odrv):
    if odrv.hw_version_major == 3:
        dma_functions = [
            [
                # https://www.st.com/content/ccc/resource/technical/document/reference_manual/3d/6d/5a/66/b4/99/40/d4/
                # DM00031020.pdf/files/DM00031020.pdf/jcr:content/translations/en.DM00031020.pdf Table 42
            ...
            [
                # https://www.st.com/content/ccc/resource/technical/document/reference_manual/3d/6d/5a/66/b4/99/40/d4/
                # DM00031020.pdf/files/DM00031020.pdf/jcr:content/translations/en.DM00031020.pdf Table 43
                [
                    "ADC1",
                    "SAI1_A",
        ...
        dma_functions = [
            [
                # https://www.st.com/resource/en/reference_manual/dm00305990-stm32f72xxx-and-stm32f73xxx-
                # advanced-armbased-32bit-mcus-stmicroelectronics.pdf Table 26
                ["SPI3_RX", "-", "SPI3_RX", "SPI2_RX", "SPI2_TX", "SPI3_TX", "-", "SPI3_TX"],
            ...
            [
                # https://www.st.com/resource/en/reference_manual/dm00305990-stm32f72xxx-and-stm32f73xxx-
                # advanced-armbased-32bit-mcus-stmicroelectronics.pdf Table 27
                [
                    "ADC1",
                    "SAI1_A",
                    "TIM8_CH1/TIM8_CH2/TIM8_CH3",
...
def dump_timing(odrv, n_samples=100, path="/tmp/timings.png"):
    ...
    for k in dir(odrv):
        if re.match(r"axis[0-9]+", k):
            for attr in dir(getattr(odrv, k).task_times):
                if not attr.startswith("_"):
                    timings.append(
                        (k + "." + attr, getattr(getattr(odrv, k).task_times, attr), [], [])
                    )  # (name, obj, start_times, lengths)
    ...
    plt.barh(
        ...
        xerr=(
            ...
            ],  # error bars to the right side  - TODO: remove artificial min()
        ),
