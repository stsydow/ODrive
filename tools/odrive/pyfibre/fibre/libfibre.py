...
fibre_cpp_paths = [
    ...
    os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(script_dir)))), "Firmware", "fibre-cpp"
    ),
]
...
if system_desc not in lib_names:
    fibre_cpp_path = get_first(fibre_cpp_paths, os.path.isdir, None)

    if fibre_cpp_path is None:
        instructions = (
            "Go to https://github.com/samuelsadok/fibre-cpp for "
            "instructions on how to compile libfibre. Once you have compiled it, "
            "add it to this folder."
        )
    else:
        instructions = (
            f"Go to {fibre_cpp_path} and run `make`. Then edit this file (libfibre.py) "
            "to include the name of the binary that was generated by `make`."
        )

    raise ModuleNotFoundError(
        "libfibre is not supported on your platform ({} {}). {}".format(*system_desc, instructions)
    )
...
if lib_path is None:
    raise ModuleNotFoundError(f"{lib_name} was not found in {search_paths}")

if os.path.getsize(lib_path) < 1000:
    raise ModuleNotFoundError(
        f"{lib_path} is too small. Did you forget to init git lfs? Try this:\n"
        " 1. Install git lfs (https://git-lfs.github.com/)\n"
        f" 2. Run `cd {os.path.dirname(lib_path)}`\n"
...
OnFunctionAddedSignature = CFUNCTYPE(
    None,
    c_void_p,
    c_void_p,
    c_void_p,
...
OnCallCompletedSignature = CFUNCTYPE(
    c_int,
    c_void_p,
    c_int,
    c_void_p,
...
if (version.major, version.minor) != (0, 1):
    raise Exception(f"Incompatible libfibre version: {version}")
...
libfibre_start_discovery.argtypes = [
    c_void_p,
    c_void_p,
    OnFoundObjectSignature,
    OnLostObjectSignature,
    OnStoppedSignature,
    c_void_p,
]
...
libfibre_subscribe_to_interface.argtypes = [
    c_void_p,
    OnAttributeAddedSignature,
    OnAttributeRemovedSignature,
    OnFunctionAddedSignature,
    OnFunctionRemovedSignature,
    c_void_p,
]
...
def _get_exception(status):
    if status == kFibreOk:
        ...
        return Exception(f"unknown libfibre error {status}")
...
def decode_arg_list(arg_names, codec_names):
    for i in count(0):
        ...
        if codec_name not in codecs:
            raise Exception(f"unsupported codec {codec_name}")
...
class TxStream:
    ...
    def write(self, data):
        ...
        libfibre_start_tx(
            self._tx_stream_handle, cast(self._tx_buf, c_char_p), len(self._tx_buf), self._c_on_tx_completed, None
        )
    ...
    async def write_all(self, data):
        ...
        while True:
            ...
            if len(data) == 0:
                ...
                raise EOFError(f"the TX stream was closed but there are still {len(data)} bytes left to send")
...
class RxStream:
    ...
    def read(self, n_read):
        ...
        libfibre_start_rx(
            self._rx_stream_handle, cast(self._rx_buf, c_char_p), len(self._rx_buf), self._c_on_rx_completed, None
        )
...
class Call:
    ...
    async def asend(self, val):
        assert self._is_started == (val is not None)
        if val is not None:
    ...
    async def __anext__(self):
        ...
        status = libfibre_call(
            ...
            cast(self._tx_buf, c_char_p),
            len(self._tx_buf),
            cast(rx_buf, c_char_p),
            len(rx_buf),
            byref(tx_end),
            byref(rx_end),
            self._func._libfibre.c_on_call_completed,
            call_id,
        )
...
class RemoteFunction:
    ...
    def __call__(self, *args, cancellation_token=None):
        ...
        if len(self._inputs) != len(args):
            raise TypeError(f"expected {len(self._inputs)} arguments but have {len(args)}")
    ...
    def _dump(self, name):
        ...
        return "{}({}){}".format(
            ...
            else " -> " + print_arglist(self._outputs)
...
class RemoteObject:
    ...
    def __setattr__(self, key, value):
        if self.__sealed__ and key not in dir(self) and not hasattr(self, key):
...
class LibFibre:
    ...
    def _load_py_intf(self, name, intf_handle):
        ...
        if intf_handle in self.interfaces:
            ...
            py_intf = self.interfaces[intf_handle] = type(
                name, (RemoteObject,), {"_handle": intf_handle, "_refcount": 0}
            )
            # exit(1)
            libfibre_subscribe_to_interface(
                intf_handle,
                self.c_on_attribute_added,
                self.c_on_attribute_removed,
                self.c_on_function_added,
                self.c_on_function_removed,
                intf_handle,
            )
            return py_intf

    def _load_py_obj(self, obj_handle, intf_handle):
        if obj_handle not in self._objects:
    ...
    def _on_attribute_added(self, ctx, attr, name, name_length, subintf, subintf_name, subintf_name_length):
        ...
        magic_getter = (
            subintf_name is not None and subintf_name.startswith("fibre.Property<") and subintf_name.endswith(">")
        )
        magic_setter = (
            subintf_name is not None
            and subintf_name.startswith("fibre.Property<readwrite ")
            and subintf_name.endswith(">")
        )
...
class _Domain:
    ...
    def _start_discovery(self):
        ...
        libfibre_start_discovery(
            self._domain_handle,
            byref(discovery._discovery_handle),
            self._libfibre.c_on_found_object,
            self._libfibre.c_on_lost_object,
            self._libfibre.c_on_discovery_stopped,
            discovery._id,
        )
        return discovery
