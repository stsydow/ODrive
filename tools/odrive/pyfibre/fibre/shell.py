...
async def discovered_device(device, interactive_variables, discovered_devices, mount, shutdown_token, logger):
    """
    ...
    logger.notify(f"{verb} to {display_name} as {var_name}")
    ...
    while len(queue):
        ...
        for key in dir(v.__class__):
            ...
            if not key.startswith("_") and isinstance(class_member, fibre.libfibre.RemoteAttribute):
                queue.append(
                    (
                        k + "." + (key if not class_member._magic_getter else "_" + key + "_property"),
                        class_member._get_obj(v),
                    )
                )
        ...
        if use_ipython:
            ...
            try:
                ...
                print(
                    ...
                    f"Run `{sudo_prefix}pip install readline` and then restart this script to fix this."
                )
