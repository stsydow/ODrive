...
def version_str_to_tuple(version_string):
    ...
    if not re.match(regex, version_string):
        raise Exception()
    return (
        int(re.sub(regex, r"\1", version_string)),
...
def get_version_from_git():
    ...
    try:
        ...
        git_tag = subprocess.check_output(
            ["git", "describe", "--always", "--tags", "--match=*fw*", "--dirty=*"], cwd=script_dir
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=argparse.FileType("w"), default="-", help="C header output file")

    args = parser.parse_args()

    git_name, major, minor, revision, unreleased = get_version_from_git()
    print("Firmware version {}.{}.{}{} ({})".format(major, minor, revision, "-dev" if unreleased else "", git_name))
    # args.output.write('const unsigned char fw_version = "{}"\n'.format(git_name))
    args.output.write(f"const unsigned char fw_version_major_ = {major};\n")
    args.output.write(f"const unsigned char fw_version_minor_ = {minor};\n")
    args.output.write(f"const unsigned char fw_version_revision_ = {revision};\n")
    args.output.write(f"const unsigned char fw_version_unreleased_ = {1 if unreleased else 0};\n")
...
def setup_udev_rules(logger):
    ...
    with open("/etc/udev/rules.d/91-odrive.rules", "w") as file:
        file.write(
            'SUBSYSTEM=="usb", ATTR{idVendor}=="1209", ATTR{idProduct}=="0d3[0-9]", MODE="0666", '
