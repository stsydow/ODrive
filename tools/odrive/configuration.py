...
def set_dict(obj, path, config_dict):
    ...
    for k, v in config_dict.items():
        ...
        if k not in dir(obj):
            errors.append(f"Could not restore {name}: property not found on device")
            continue
...
def get_temp_config_filename(device):
    ...
    return os.path.join(tempfile.gettempdir(), f"odrive-config-{safe_serial_number}.json")
...
def backup_config(device, filename, logger):
    ...
    if os.path.exists(filename):
        if not yes_no_prompt(f"The file {filename} already exists. Do you want to override it?", True):
...
def restore_config(device, filename, logger):
    ...
    logger.info(f"Restoring configuration from {filename}...")
