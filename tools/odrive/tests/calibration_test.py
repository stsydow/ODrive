...
class TestMotorCalibration:
    ...
    def run_test(self, axis_ctx: ODriveAxisComponent, motor_ctx: MotorComponent, logger: Logger):
        ...
        test_assert_eq(
            axis_ctx.handle.motor.config.phase_resistance, float(motor_ctx.yaml["phase-resistance"]), accuracy=0.2
        )
        test_assert_eq(
            axis_ctx.handle.motor.config.phase_inductance, float(motor_ctx.yaml["phase-inductance"]), accuracy=0.5
        )
...
class TestEncoderIndexSearch:
    ...
    def get_test_cases(self, testrig: TestRig):
        for axis, motor, encoder, tf1 in testrig.get_closed_loop_combos(init=False):
            alternatives = []
            for z_gpio, tf2 in testrig.get_connected_components(
                (axis.parent.encoders[axis.num].z, False), LinuxGpioComponent
            ):
