        ...
        for odrive in testrig.get_components(ODriveComponent):
            if odrive.yaml["board-version"].startswith("v3."):
                ports = testrig.get_connected_components(
                    {"rx": (odrive.gpio1, True), "tx": (odrive.gpio2, False)}, SerialPortComponent
                )
                ...
                ports = list(
                    testrig.get_connected_components(
                        {"rx": (odrive.gpio15, True), "tx": (odrive.gpio14, False)}, SerialPortComponent
                    )
                )
...
class TestUartAscii(UartTest):
    ...
    def run_test(
        ...
        with port.open(115200) as ser:
            ...
            response = float(ser.readline().strip())
            test_assert_eq(
                response, odrive.handle.axis0.motor.current_control.v_current_control_integral_d, accuracy=0.1
            )
...
class TestUartNoise(UartTest):
        ...
        for odrive in testrig.get_components(ODriveComponent):
            ...
            for teensy in testrig.get_components(TeensyComponent):
                for port in testrig.get_components(SerialPortComponent):
                    gpio_conns = [
                        testrig.net_by_component.get(getattr(odrive, f"gpio{tx_gpio}"), set()).intersection(
                            set(teensy.gpios)
                        ),
                        testrig.net_by_component.get(getattr(odrive, f"gpio{rx_gpio}"), set()).intersection(
                            set(teensy.gpios)
                        ),
    ...
    def run_test(
        ...
        rx_gpio: list,
        port: SerialPortComponent,
        noise_enable: LinuxGpioComponent,
        logger: Logger,
    ):
