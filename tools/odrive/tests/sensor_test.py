...
class TestVbusVoltage:
    ...
    def run_test(self, odrive: ODriveComponent, logger: Logger):
        ...
        test_curve_fit(
            data[:, (0, 1)],
            fitted_curve,
            max_mean_err=expected_voltage * 0.01,
            inlier_range=expected_voltage * 0.02,
...
class TestOnboardTempSensors:
    ...
    def run_test(self, axis: ODriveAxisComponent, logger: Logger):
        ...
        test_assert_eq(slope, 0, range=0.01)
        test_curve_fit(
            data[:, (0, 1)], fitted_curve, max_mean_err=0.2, inlier_range=0.5, max_outliers=len(data[:, 0]) * 0.01
        )
...
class TestOffboardTempSensors:
    ...
    def get_test_cases(self, testrig: TestRig):
        for odrive in testrig.get_components(ODriveComponent):
            if odrive.yaml["board-version"].startswith("v4."):
                ...
                yield AnyTestCase(
                    *[(odrive.axes[0], teensy_gpio.parent, teensy_gpio, tf) for teensy_gpio, tf in teensy_gpios]
                )
    ...
    def run_test(self, axis: ODriveAxisComponent, teensy: TeensyComponent, teensy_gpio: TeensyGpio, logger: Logger):
        ...
        set_motor_thermistor_coeffs(
            axis.handle, Rload=1000, R_25=10000, Beta=3434, Tmin=0, Tmax=140, thermistor_bottom=True
        )
