...
def test_assert_eq(observed, expected, range=None, accuracy=None):
    ...
    if range is not None:
        if (observed < expected - range) or (observed > expected + range):
            raise TestFailed(f"value out of range: expected {expected}+-{range} but observed {observed}")

    # Comparision with relative range
    elif accuracy is not None:
        if (
            sign(observed) != sign(expected)
            or (abs(observed) < abs(expected) * (1 - accuracy))
            or (abs(observed) > abs(expected) * (1 + accuracy))
        ):
            raise TestFailed(f"value out of range: expected {expected}+-{accuracy * 100.0}% but observed {observed}")

    # Exact comparision
    else:
        if observed != expected:
            raise TestFailed(f"value mismatch: expected {expected} but observed {observed}")
...
def test_curve_fit(data, fitted_curve, max_mean_err, inlier_range, max_outliers):
    ...
    if mean_err > max_mean_err:
        ...
        raise TestFailed(f"curve fit has too large mean error: {mean_err} > {max_mean_err}")

    outliers = np.count_nonzero((diffs > inlier_range) | (diffs < -inlier_range))
    if outliers > max_outliers:
        save_log(np.concatenate([data, np.array([fitted_curve]).transpose()], 1))
        raise TestFailed(f"curve fit has too many outliers (err > {inlier_range}): {outliers} > {max_outliers}")
...
def test_watchdog(axis, feed_func, logger: Logger):
    ...
    for _ in range(5):  # keep the watchdog alive for 3.5 seconds
        time.sleep(0.7)
        logger.debug(f"feeding watchdog at {time.monotonic() - start}s")
        feed_func()
        err = axis.error
        logger.debug(f"checking error at {time.monotonic() - start}s")
...
class ODriveComponent(Component):
    def __init__(self, yaml: dict):
        ...
        if yaml["board-version"].startswith("v3."):
            self.encoders = [
                ODriveEncoderComponent(self, 0, yaml["encoder0"]),
                ODriveEncoderComponent(self, 1, yaml["encoder1"]),
            ]
    ...
    def prepare(self, logger: Logger):
        ...
        self.handle = odrive.find_any(
            channel_termination_token=shutdown_token, serial_number=self.yaml["serial-number"], timeout=60
        )  # , printer=print)
        assert self.handle
...
class LinuxGpioComponent(Component):
    ...
    def config(self, output: bool):
        with open(f"/sys/class/gpio/gpio{self.num}/direction", "w") as fp:
            fp.write("out" if output else "0")

    def write(self, state: bool):
        with open(f"/sys/class/gpio/gpio{self.num}/value", "w") as fp:
...
class TeensyComponent(Component):
    ...
    def commit_routing_config(self, logger: Logger):
        ...
        for i, o, n in self.routes:
            if n:
                # with noise enable
                code += f"  digitalWrite({o.num}, digitalRead({n.num}) ? noise : digitalRead({i.num}));\n"
            else:
                # no noise enable
                code += f"  digitalWrite({o.num}, digitalRead({i.num}));\n"
        code += "}\n"
    ...
    def compile(self, sketchfile, hexfile):
        ...
        run_shell(
            ["arduino", "--board", self.yaml["board-version"], "--verify", sketchfile], logger, env=env, timeout=120
        )
    ...
    def program(self, hex_file_path: str, logger: Logger):
        ...
        program_gpio = self.testrig.get_directly_connected_components(
            self.testrig.get_component_name(self) + ".program"
        )[0]
        ...
        run_shell(
            [
                "teensy-loader-cli",
                "-mmcu=" + self.yaml["board-version"].rpartition(":")[2].upper(),
                "-w",
                hex_file_path,
            ],
            logger,
...
class TestFixture:
    ...
    def all_of(*test_fixtures):
        test_fixtures = [
            (tf._subfixtures if isinstance(tf, CompositeTestFixture) else [[tf]])
    ...
    def any_of(*test_fixtures):
        test_fixtures = [(tf._subfixtures if isinstance(tf, CompositeTestFixture) else [[tf]]) for tf in test_fixtures]
...
class TestRig:
    ...
    def get_directly_connected_components(self, component: str | Component):
        """
        ...
        if isinstance(src, dict):
            for candidate in candidates:
                ...
                for name, subsrc in src.items():
                    ...
                    is_connected, test_fixture = (
                        self.check_connection(srcport, subcomponents[name])
                        if direction
                        else self.check_connection(subcomponents[name], srcport)
                    )
                    if not is_connected:
                        break
            ...
            for candidate in candidates:
                is_connected, test_fixture = (
                    self.check_connection(src, candidate) if direction else self.check_connection(candidate, src)
                )
                if is_connected:
...
def test_assert_no_error(axis_ctx: ODriveAxisComponent):
    any_error = (
        axis_ctx.handle.motor.error
        | axis_ctx.handle.encoder.error
        |
        # axis_ctx.handle.sensorless_estimator.error | # TODO: reenable
        axis_ctx.handle.error
    ) != 0  # TODO: this is not the complete list of components
...
def run_shell(command_line, logger, env=None, timeout=None):
    ...
    result = subprocess.run(cmd, timeout=timeout, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env)
    if result.returncode != 0:
        logger.error(result.stdout.decode(sys.stdout.encoding))
        raise TestFailed(f"command {command_line} failed")
...
def is_feasible(params, test_fixtures):
    ...
    for r, ex in [(r, ex) for tf in test_fixtures if tf is not None for r, ex in tf.get_resources() if r is not None]:
        if ex:
            exclusive_tf_resources.append(r)
        else:
            shared_tf_resources.add(r)
    if len(exclusive_tf_resources + list(shared_tf_resources)) > len(
        set(exclusive_tf_resources).union(shared_tf_resources)
    ):
...
parser.add_argument("--all", action="store_true", default=False, help="Run all tests in the test runner's directory")
parser.add_argument("--html", type=str, help="If provided, the an HTML summary is written to the specified file.")
...
if args.test_rig_yaml is None:
    ...
    if test_rig_name == "":
        ...
        sys.exit(1)
    path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))), test_rig_name + ".yaml"
    )
    args.test_rig_yaml = open(path)
...
if args.setup_host:
    for gpio in testrig.get_components(LinuxGpioComponent):
        ...
        if not os.path.isdir(f"/sys/class/gpio/gpio{num}"):
            with open("/sys/class/gpio/export", "w") as fp:
                fp.write(str(num))
        os.chmod(f"/sys/class/gpio/gpio{num}/value", stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
        os.chmod(f"/sys/class/gpio/gpio{num}/direction", stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
    ...
    if len(list(testrig.get_components(TeensyComponent))):
        ...
        if not os.path.isfile("/usr/share/arduino/hardware/tools/teensy_post_compile_old"):
            os.rename(
                "/usr/share/arduino/hardware/tools/teensy_post_compile",
                "/usr/share/arduino/hardware/tools/teensy_post_compile_old",
            )
    ...
    for intf in testrig.get_components(CanInterfaceComponent):
        ...
        logger.debug(f"bringing up {name}...")
        run_shell(f"ip link set dev {name} down", logger)
        if path is not None:
            run_shell(f"slcand -o -c -s5 '{path}' {name}", logger)
        else:
            run_shell(f"ip link set dev {name} type can bitrate 250000", logger)
            run_shell(f"ip link set dev {name} type can loopback off", logger)
        run_shell(f"ip link set dev {name} up", logger)
...
if __name__ == "__main__":
    ...
    if args.all:
        test_scripts = [file for file in os.listdir(os.path.dirname(__file__)) if file.lower().endswith("_test.py")]
