...
def get_errors(axis_ctx: AxisTestContext):
    ...
    if axis_ctx.handle.motor.error != 0:
        errors.append(f"motor failed with error 0x{axis_ctx.handle.motor.error:04X}")
    if axis_ctx.handle.encoder.error != 0:
        errors.append(f"encoder failed with error 0x{axis_ctx.handle.encoder.error:04X}")
    if axis_ctx.handle.sensorless_estimator.error != 0:
        errors.append(f"sensorless_estimator failed with error 0x{axis_ctx.handle.sensorless_estimator.error:04X}")
    if axis_ctx.handle.error != 0:
        errors.append(f"axis failed with error 0x{axis_ctx.handle.error:04X}")
    elif len(errors) > 0:
...
def set_limits(axis_ctx: AxisTestContext, logger, vel_limit=20000, current_limit=10):
    ...
    logger.debug(f"velocity limit = {vel_limit} => V_emf = {max_emf_voltage:.3}V, I_lim = {max_motor_current:.3}A")
...
class AxisTest(ABC):
    ...
    def check_preconditions(self, axis_ctx: AxisTestContext, logger):
        ...
        test_assert_eq(
            axis_ctx.odrv_ctx.handle.config.dc_bus_undervoltage_trip_level,
            axis_ctx.odrv_ctx.yaml["vbus-voltage"] * 0.85,
            accuracy=0.001,
        )
        test_assert_eq(
            axis_ctx.odrv_ctx.handle.config.dc_bus_overvoltage_trip_level,
...
class TestSetup(ODriveTest):
    ...
    def run_test(self, odrv_ctx: ODriveTestContext, logger):
        ...
        test_assert_eq(
            odrv_ctx.handle.config.dc_bus_undervoltage_trip_level, odrv_ctx.yaml["vbus-voltage"] * 0.85, accuracy=0.001
        )
        test_assert_eq(
            odrv_ctx.handle.config.dc_bus_overvoltage_trip_level, odrv_ctx.yaml["vbus-voltage"] * 1.08, accuracy=0.001
        )
...
class TestMotorCalibration(AxisTest):
    ...
    def check_preconditions(self, axis_ctx: AxisTestContext, logger):
        super().check_preconditions(axis_ctx, logger)
    ...
    def run_test(self, axis_ctx: AxisTestContext, logger):
        ...
        test_assert_no_error(axis_ctx)
        test_assert_eq(
            axis_ctx.handle.motor.config.phase_resistance, axis_ctx.yaml["motor-phase-resistance"], accuracy=0.2
        )
        test_assert_eq(
            axis_ctx.handle.motor.config.phase_inductance, axis_ctx.yaml["motor-phase-inductance"], accuracy=0.5
        )
...
class TestClosedLoopControl(AxisTest):
    ...
    def run_test(self, axis_ctx: AxisTestContext, logger):
        ...
        expected_sensorless_estimation = (
            40000 * 2 * math.pi / axis_ctx.yaml["encoder-cpr"] * axis_ctx.yaml["motor-pole-pairs"]
        )
...
class TestHighVelocity(AxisTest):
    ...
    def run_test(self, axis_ctx: AxisTestContext, logger):
        ...
        logger.debug(f"rated max speed: {rated_limit}, expected max speed: >= {expected_limit}")
        ...
        logger.debug(f"Drive current {axis_ctx.handle.motor.config.current_lim}A, Load current {self._load_current}A")

        ramp_up_time = 15.0
        max_measured_vel = 0.0
        logger.debug(f"ramping to {rated_limit} over {ramp_up_time} s")
        t_0 = time.monotonic()
        last_print = t_0
        while True:
            ...
            if time.monotonic() - last_print > 1:
                ...
                logger.debug(
                    f"ramping up: commanded {vel_setpoint}, expected {expected_velocity}, measured {measured_vel} "
                )

            time.sleep(0.001)

        logger.debug(f"reached top speed of {max_measured_vel} counts/sec")
    ...
    def run_test(self, axis0_ctx: AxisTestContext, axis1_ctx: AxisTestContext, logger):
        ...
        if driver_ctx.name == "top-odrive.black":
            ...
            odrive.utils.start_liveplotter(
                lambda: [
                    driver_ctx.handle.motor.current_control.Iq_measured,
                    driver_ctx.handle.motor.current_control.Iq_setpoint,
                ]
            )

        # Set up viscous fluid load
        logger.debug(f"activating load on {load_ctx.name}...")
        ...
        driver_test = TestHighVelocity(
            override_current_limit=self._driver_current, load_current=self._load_current, brake=False
        )
...
class TestSelfLoadedPosVelDistribution(DualAxisTest):
    ...
    def run_test(self, axis0_ctx: AxisTestContext, axis1_ctx: AxisTestContext, logger):
        ...
        logger.debug(f"Iload range: {self._load_current_range} A, Idriver: {self._driver_current_lim} A")
        ...
        logger.debug(f"RPM range: {max_rpm} = driver {driver_max_speed} = load {load_max_speed}")
        ...
        logger.debug(f"activating driver on {driver_ctx.name}")
        ...
        while t_ratio < 1:
            ...
            if time.monotonic() - last_print > 1:
                last_print = time.monotonic()
                logger.debug(
                    f"Envelope  --  vel: {t_ratio * driver_max_speed:.2f}, I: {t_ratio * self._load_current_range:.2f}"
                )
...
class TestVelCtrlVsPosCtrl(DualAxisTest):
    ...
    def run_test(self, axis0_ctx: AxisTestContext, axis1_ctx: AxisTestContext, logger):
        ...
        logger.debug(f"using {driver_ctx.name} as driver against load, vel=100000...")
        ...
        logger.debug(f"using {driver_ctx.name} as driver against load, vel=20000...")
        ...
        odrive.utils.print_drv_regs(f"load motor ({load_ctx.name})", load_ctx.handle.motor)
        odrive.utils.print_drv_regs(f"driver motor ({driver_ctx.name})", driver_ctx.handle.motor)
...
class TestAsciiProtocol(ODriveTest):
    def run_test(self, odrv_ctx: ODriveTestContext, logger):
        ...
        if expected_line not in lines:
            raise Exception(f"expected {expected_line} in ASCII protocol response but got {lines!s}")
