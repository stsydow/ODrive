...
command_set = {
    "heartbeat": (
        0x001,
        [("error", "I", 1), ("current_state", "B", 1), ("reserved", "H", 1), ("controller_state", "B", 1)],
    ),  # tested
    "estop": (0x002, []),  # tested
...
class TestSimpleCAN:
    ...
    def run_test(
        self, odrive: ODriveComponent, canbus: CanInterfaceComponent, node_id: int, extended_id: bool, logger: Logger
    ):
        ...
        test_assert_eq(
            my_req("get_encoder_estimates")["encoder_pos_estimate"], 123.0 / axis.encoder.config.cpr, accuracy=0.01
        )
        ...
        heartbeats = asyncio.run(
            get_all(record_messages(canbus.handle, node_id, extended_id, "heartbeat", timeout=2.0))
        )
        ...
        my_cmd("reboot")
        time.sleep(0.5)
        if odrive.handle._on_lost is not None:
