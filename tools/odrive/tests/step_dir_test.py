...
class TestStepDir:
        ...
        for odrive in testrig.get_components(ODriveComponent):
            ...
            def stepdir_test_case(axis, step_gpio_num, dir_gpio_num, odrive=odrive):
                ...
                for step_ctrl_gpio, tf1 in testrig.get_connected_components(
                    ...
                    for dir_ctrl_gpio, tf2 in testrig.get_connected_components(
                        (getattr(odrive, f"gpio{dir_gpio_num}"), False), LinuxGpioComponent
                    ):
                        alternatives.append(
                            (
                                axis,
    ...
    def run_test(
        self,
        axis: ODriveAxisComponent,
        step_gpio_num: int,
        step_gpio: LinuxGpioComponent,
        dir_gpio_num: int,
        dir_gpio: LinuxGpioComponent,
        logger: Logger,
    ):
