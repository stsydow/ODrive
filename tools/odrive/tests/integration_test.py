...
command_set = {
    ...
    "get_encoder_estimates": (
        0x009,
        [("encoder_pos_estimate", "f", 1), ("encoder_vel_estimate", "f", 1)],
    ),  # partially tested
    ...
    "get_sensorless_estimates": (
        0x015,
        [("sensorless_pos_estimate", "f", 1), ("sensorless_vel_estimate", "f", 1)],
    ),  # untested
    "reboot": (0x016, []),  # tested
...
async def record_messages(bus, node_id, extended_id, cmd_name, timeout=5.0):
    ...
    try:
        ...
        while True:
            ...
            if (
                (msg.arbitration_id == ((node_id << 5) | cmd_id))
                and (msg.is_extended_id == extended_id)
...
class TestSimpleCANClosedLoop:
    def prepare(
        ...
        canbus: CanInterfaceComponent,
        axis_ctx: ODriveAxisComponent,
        motor_ctx: MotorComponent,
        enc_ctx: EncoderComponent,
        node_id: int,
        extended_id: bool,
        logger: Logger,
    ):
        ...
        while (
            axis_ctx.handle.requested_state != AXIS_STATE_UNDEFINED or axis_ctx.handle.current_state != AXIS_STATE_IDLE
        ):
            time.sleep(1)
    ...
    def run_test(
        ...
        with self.prepare(odrive, canbus, axis_ctx, motor_ctx, enc_ctx, node_id, extended_id, logger):
            ...
            def fence():
                my_req("get_vbus_voltage")  # fence to ensure the CAN command was sent

            def flush_rx():
                while canbus.handle.recv(timeout=0) is not None:
                    pass
            ...
            my_cmd(
                "set_controller_modes", control_mode=CONTROL_MODE_POSITION_CONTROL, input_mode=INPUT_MODE_PASSTHROUGH
            ...
            my_cmd(
                "set_controller_modes", control_mode=CONTROL_MODE_VELOCITY_CONTROL, input_mode=INPUT_MODE_PASSTHROUGH
            ...
            time.sleep(5)
            test_assert_eq(
                axis_ctx.handle.encoder.vel_estimate, nominal_vel, range=nominal_vel * 0.05
            )  # big range here due to cogging and other issues
            ...
            my_cmd(
                "set_controller_modes", control_mode=CONTROL_MODE_TORQUE_CONTROL, input_mode=INPUT_MODE_PASSTHROUGH
