...
class TestClosedLoopControl(TestClosedLoopControlBase):
    ...
    def run_test(
        ...
        with SafeTerminator(logger, axis_ctx):
            ...
            data = record_log(
                lambda: [axis_ctx.handle.encoder.vel_estimate, axis_ctx.handle.encoder.pos_estimate], duration=5.0
            )
            ...
            logger.debug("Testing closed loop position control...")
            ...
            test_assert_no_error(axis_ctx)
            test_assert_eq(
                axis_ctx.handle.encoder.pos_estimate, test_pos, range=0.4 * test_pos
            )  # large range needed because of cogging torque
            ...
            data = record_log(
                lambda: [axis_ctx.handle.encoder.vel_estimate, axis_ctx.handle.encoder.pos_estimate], duration=4.0
            )
...
class TestRegenProtection(TestClosedLoopControlBase):
    ...
    def run_test(
        ...
        with SafeTerminator(logger, axis_ctx):
            ...
            logger.debug("Brake control test with brake resistor disabled")
            ...
            logger.debug("Brake control test with brake resistor = 100")
...
class TestVelLimitInTorqueControl(TestClosedLoopControlBase):
    ...
    def run_test(
        ...
        with SafeTerminator(logger, axis_ctx):
            ...
            axis_ctx.handle.controller.config.vel_gain /= (
                10  # reduce the slope to make it easier to see what's going on
            )
            ...
            def get_expected_setpoint(input_setpoint, velocity):
                return (
                    clamp(
                        clamp(
                            input_setpoint / torque_constant,
                            (velocity + max_vel) * -vel_gain / torque_constant,
                            (velocity - max_vel) * -vel_gain / torque_constant,
                        ),
                        -max_current,
                        max_current,
            ...
            test_curve_fit(
                dataB[:, (0, 3)], dataB[:, 4], max_mean_err=0.1, inlier_range=0.3, max_outliers=len(dataB[:, 0]) * 0.01
            )
...
tests = [TestClosedLoopControl(), TestRegenProtection(), TestVelLimitInTorqueControl(), TestTorqueLimit()]
