...
class FightingTest:
    def get_test_cases(self, testrig: TestRig):
        ...
        for (axis0, motor0, encoder0, test_fixture0), (
            axis1,
            motor1,
            encoder1,
            test_fixture1,
        ) in itertools.combinations(all_combos, 2):
    ...
    def run_test(
        self,
        axis0_ctx: ODriveAxisComponent,
        motor0_ctx: MotorComponent,
        enc0_ctx: EncoderComponent,
        axis1_ctx: ODriveAxisComponent,
        motor1_ctx: MotorComponent,
        enc1_ctx: EncoderComponent,
        logger: Logger,
    ):
        ...
        with SafeTerminator(logger, axis0_ctx, axis1_ctx):
            ...
            data = record_log(
                lambda: [
                    axis0_ctx.handle.encoder.vel_estimate,
                    axis1_ctx.handle.encoder.vel_estimate,
                    axis0_ctx.handle.encoder.pos_estimate,
                ],
                duration=5.0,
            )
