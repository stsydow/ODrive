...
class ACMotor:
    ...
    def system_function(self, t, y):
        ...
        leakage_factor = 1.0 - mutual_inductance**2 / (
            p[pl["rotor_inductance"]] * p[pl["stator_inductance"]]
        )  # aka sigma [unitless]
        ...
        dstator_current_dt = (
            ...
            - coupling_factor
            / (r_sigma * tau_rotor)
            * (1.0j * self.omega_rotor * tau_rotor - 1.0)
            * y[sl["rotor_flux"]]
    ...
    def print_parameter_info(self):
        ...
        print(f"pole_pairs = {EngNumber(pole_pairs)}")
        print(f"rotor_resistance = {EngNumber(assumed_rotor_resistance)}ohm")

        print()
        print("Fitted parameters:")
        for i, r in enumerate(ACMotor.parameter_definitions):
            print(f"{r[0]} = {EngNumber(self.params[i])}{r[2]}")
        ...
        print(f"mutual_inductance = {EngNumber(mutual_inductance)}H")
        print(f"coupling_factor = {EngNumber(coupling_factor)}")
        print(f"torque_constant = {EngNumber(torque_constant)}Nm/A^2")
        print(f"motor_constant = {EngNumber(motor_constant)}Nm/W")
    ...
    def print_run_info(self, y):
        ...
        print()
        print("Final values:")
        print(f"final_rotor_flux_d = {EngNumber(final_rotor_flux_d)}Wb")
        print(f"final_stator_current_d = {EngNumber(final_stator_current_d)}A")
        print(f"final_stator_current_q = {EngNumber(final_stator_current_q)}A")
        print(f"final_torque_per_q_amp = {EngNumber(final_torque_per_q_amp)}Nm/A")
...
if PLOT_INITAL:
    ...
    y = motor.run(time_series=t, voltage=voltage_step, omega_stator=0, omega_rotor=0)
