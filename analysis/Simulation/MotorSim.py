...
class motor:
    ...
    def diff_eqs(self, t, y):
        ...
        I_d_dot = (
            self.V_d / self.L_d - self.R / self.L_d * I_d + theta_dot * self.pole_pairs * self.L_q / self.L_d * I_q
        )
        I_q_dot = (
            self.V_q / self.L_q
            - self.R / self.L_q * I_q
            - theta_dot * self.pole_pairs * self.L_d / self.L_q * I_d
            - theta_dot * self.pole_pairs * self.lambda_m / self.L_q
        )
    ...
    def single_step_rk(self, V_q, V_d, T_load):
        ...
        ((d5065.theta, d5065.theta_dot, d5065.I_d, d5065.I_q), _) = rk_step(
            d5065.diff_eqs, 0, x, d5065.diff_eqs(0, x), d5065.dT, A, B, C, d5065.K
        )


if __name__ == "__main__":
    d5065 = motor(
        J=1e-4, b_coulomb=0, b_viscous=0.01, R=0.039, L_q=1.57e-5, L_d=1.57e-5, KV=270, pole_pairs=7, dT=1 / 48000
    )
