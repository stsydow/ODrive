...
echo "==> mypy (main, backend, monitoring, status_backend, errors, eventlog, configtree)"
mypy --no-incremental --ignore-missing-imports \
     main.py backend.py monitoring.py status_backend.py errors.py eventlog.py configtree.py || fail=1

echo "==> python -m py_compile (all modules)"
python3 -m py_compile main.py backend.py monitoring.py status_backend.py errors.py eventlog.py configtree.py || fail=1
