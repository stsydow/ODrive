...
def decode_error_summary(report) -> str:
    ...
    for s in report.sources:
        if not s.value:
            continue
        # Module name -> error-enum prefix; "system" and "sensorless" do not
        # follow the name.upper() pattern (read_error_report uses the same
        # prefixes when it builds s.errors).
        prefix = {
            "system": "ODRIVE_ERROR_",
            "sensorless": "SENSORLESS_ESTIMATOR_ERROR_",
        }.get(s.name, f"{s.name.upper()}_ERROR_")
        for err in s.errors:
