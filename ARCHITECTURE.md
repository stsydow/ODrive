...
- **Safety Arm Latch for Autonomous/Pedal Inputs**: In ODrive, `AXIS_STATE_IDLE` is both the unpowered state and the error-recovery fallback state. Any autonomous or analog-input transition out of `IDLE` (`deadband_idle`) requires an active arming latch (`deadband_armed`). Errors or explicit stop commands immediately clear the latch so cleared faults or rebooted boards never spontaneously engage.
- **Fibre Pointer Bounds for Parent Resolution**: Firmware Fibre endpoints lack back-pointers to containing parent objects. Resolving ownership (e.g. mapping an endpoint ref to its parent `Axis`) is accomplished via pointer-range checking on the introspection context (`Property::ctx_`) against contiguous allocations of `axes[i]` and constituent objects (`controllers[i]`, `motors[i]`, etc.), avoiding fragile hardcoded endpoint tables.
- **ODrive v3 NVM Save is a System Reset**: `odrv.save_configuration()` immediately writes to flash and triggers a hardware reset on STM32. Host tooling must treat the resulting `ObjectLostError` / USB disconnect as the normal success signal rather than an unhandled error.
